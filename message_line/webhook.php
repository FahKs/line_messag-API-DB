...
function verifyUserByEmail($replyToken, $email, $userId, $conn, $accessToken) {
    try {
        ...
        if ($result->num_rows > 0) {
            ...
            $message = "ไม่พบอีเมลนี้ในระบบ กรุณาลงทะเบียนก่อนใช้งานได้ที่นี่:https://7b49-125-26-7-18.ngrok-free.app/NT/page/login.php";
            sendReply($replyToken, $message, $accessToken);
        }
    } catch (Exception $e) {
...
function showBillDetails($replyToken, $billNumber, $conn, $accessToken) { 
    ...
    // 3. ดึงข้อมูลบริการและราคา Overide
    $sqlServices = "
        SELECT 
            s.code_service,
            s.type_service,
            ...
            o.ict_price,
            o.all_price
        FROM service_customer s
        LEFT JOIN package_list p ON s.id_service = p.id_service
        LEFT JOIN product_list pr ON p.id_package = pr.id_package
    ...
    if ($resultServices && $resultServices->num_rows > 0) {
        ...
        while ($service = $resultServices->fetch_assoc()) {
            ...
            if (isset($service['ict_price'])) {
                $message .= "  ราคา ICT: " . number_format($service['ict_price'], 2) . " บาท\n";
            }
